...
export class ElectronRunner extends Runner {
  ...
  private _setupTray(): void {
    ...
    this._tray.on('click', (event, bounds, position) => {
      this._customMenu.isVisible()
        ? this._customMenu.hide()
        : this._showMenu(bounds, position);
    });
  }

  private _setupIpc(): void {
    ipcMain.handle(
      MenuRenderKeys.uniqMatchId,
      (_, event: TennisEvent, match: TennisMatch) =>
        this.uniqMatchId(event, match)
    );
    ipcMain.on(MenuRenderKeys.setMatchSelected, (_, matchId: string) =>
      this._selectedMatchIds.add(matchId)
    );
  ...
  private _showMenu(bounds: Rectangle, position: Point) {
    ...
    if (process.platform === 'darwin') {
      // macOS (top menu bar)
      x = Math.round(
        iconBounds.x + iconBounds.width / 2 - windowBounds.width / 2
      );
      y = iconBounds.y + iconBounds.height;
    } else {
      // Windows / Linux (usually bottom taskbar)
      x = Math.round(
        iconBounds.x + iconBounds.width / 2 - windowBounds.width / 2
      );
    ...
    ipcMain.on(
      MenuRenderKeys.resizeToFitContents,
      async (event, width: number) => {
        const [, windowHeight] = this._customMenu.getSize();
        this._customMenu.setSize(
          Math.max(width, ElectronRunner.DefaultWindowWidth),
          Math.max(windowHeight, ElectronRunner.DefaultWindowHeight)
        );
      }
    );
  }
  ...
  updateLastRefreshTime(): void {
    if (this._customMenu) {
      this._customMenu.webContents.send(
        MenuRenderKeys.updateLastRefreshTime,
        this.lastRefreshTimeDisplay()
      );
    }
  }
  ...
  addEventMenuItem(
    ...
    this._customMenu.webContents.send(
      MenuRenderKeys.addEventMenuItem,
      event,
      text,
      position,
      url,
  ...
  addMatchMenuItem(
    ...
    this._customMenu.webContents.send(
      MenuRenderKeys.addMatchMenuItem,
      event,
      match,
      isSelected
    );
  ...
  updateMatchMenuItem(matchId: string, match: TennisMatch): void {
    this._customMenu.webContents.send(
      MenuRenderKeys.updateMatchMenuItem,
      matchId,
      match
    );
  }
  ...
  setMatchSelection(matchId: string, selection: boolean): void {
    this._customMenu.webContents.send(
      MenuRenderKeys.setMatchSelection,
      matchId,
      selection
    );
  }
  ...
  removeEventMenuItem(event: TennisEvent): void {
    this._customMenu.webContents.send(
      MenuRenderKeys.removeEventMenuItem,
      event
    );
  }
  ...
  removeMatchMenuItem(matchId: string): void {
    this._customMenu.webContents.send(
      MenuRenderKeys.removeMatchMenuItem,
      matchId
    );
  }
