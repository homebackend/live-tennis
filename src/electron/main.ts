...
async function addAutostartIfApplicable(
...
function createMainWindow(
  ...
  const updater = new LiveViewUpdater(
    runner,
    manager,
    apiHandlers,
    settings,
  ...
  ipcMain.handle(
    MenuRenderKeys.getSettingBoolean,
    async (_, key: string) => await settings.getBoolean(key)
  );
  ipcMain.on(
    MenuRenderKeys.setSettingBoolean,
    async (_, key: string, value: boolean) => {
      await settings.setBoolean(key, value);
      handleSettingChange(key);
    }
  );
  ipcMain.handle(
    MenuRenderKeys.getSettingInt,
    async (_, key: string) => await settings.getInt(key)
  );
  ipcMain.on(
    MenuRenderKeys.setSettingInt,
    async (_, key: string, value: number) => {
      await settings.setInt(key, value);
      handleSettingChange(key);
    }
  );
  ipcMain.handle(
    MenuRenderKeys.getSettingStrV,
    async (_, key: string) => await settings.getStrv(key)
  );
  ipcMain.on(
    MenuRenderKeys.setSettingStrv,
  ...
  ipcMain.on(
    MenuRenderKeys.openSettingsWindow,
    () => new PrefsManager(__dirname)
  );
...
app.whenReady().then(() => {
  ...
  settings.getBoolean('enable-debug-logging').then((debug) => {
    ...
    checkForUpdateAndStart(splash, log, () => createMainWindow(settings, log));
  });
});
