...
interface NonBooleanSchemaItem<
  K extends keyof Schema,
> extends BaseSchemaItem<K> {
  dependent?: never;
}

interface NonNumericSchemaItem<
  K extends keyof Schema,
> extends BaseSchemaItem<K> {
  increment?: never;
  minimum?: never;
...
export type FullSchemaItem<K extends keyof Schema = keyof Schema> =
  K extends BooleanKeysOfSchema
    ? BooleanSchemaItem<K>
    : K extends NumericKeysOfSchema
      ? NumericSchemaItem<K>
      : K extends ArrayKeyOfSchema
        ? ArraySchemaItem<K>
        : never;
...
export const schema: FullSchema = {
  autostart: {
    ...
    description:
      'Whether to start Live Tennis automatically with your desktop.',
  },
  enabled: {
    type: 'boolean',
    default: false,
  ...
  num_windows: {
    ...
    applicability: [
      SettingApplicability.ElectronTrayApp,
  ...
  match_display_duration: {
    ...
    description:
      'The duration in seconds to display a single match before cycling to the next one, if multiple are selected.',
  },
  selected_matches: {
    type: 'array',
    items: {
  ...
  auto_view_new_matches: {
    ...
    description:
      'An array of event IDs for new matches are automatically added to live view.',
  },
  auto_select_live_matches: {
    ...
    description:
      'Whether to automatically select matches currently in progress.',
  },
  ...
  auto_select_player_names: {
    ...
    description:
      'A comma-separated list of player names. Each name must contain only alphabetic characters. The name could be first name or last name.',
  },
  keep_completed_duration: {
    ...
    description:
      'The duration in minutes to keep matches visible after they have finished.',
  },
  enable_atp: {
    type: 'boolean',
    default: true,
  ...
  atp_update_interval: {
    ...
    description:
      'The interval in seconds at which to fetch new match data from ATP.',
  },
  wta_update_interval: {
    type: 'number',
    default: 90,
    ...
    description:
      'The interval in seconds at which to fetch new match data from WTA.',
  },
  atp_challenger_update_interval: {
    ...
    description:
      'The interval in seconds at which to fetch new match data from ATP Challenger.',
  },
  tennis_temple_update_interval: {
    ...
    description:
      'The interval in seconds at which to fetch new match data from Tennis Temple.',
  },
  live_window_size_x: {
    type: 'number',
    default: 450,
