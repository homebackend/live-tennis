  ...
  private async _fetch(
    ...
    const requestConfig: AxiosRequestConfig = {
      ...
      data: request.payload
        ? Array.from(request.payload.entries())
            .map(
              ([key, value]) =>
    ...
    try {
      ...
      if (cookiesHeader && request.responseCookies) {
        ...
        cookiesHeader
          ...
          .filter(
            (values) =>
              values.length >= 2 && request.responseCookies?.includes(values[0])
          )
          .forEach(([key, value]) => responseCookies!.set(key, value));

        this._log([
          'Parsed response cookies:',
          JSON.stringify(Object.fromEntries(responseCookies)),
        ]);
      }
      ...
      if (axios.isCancel(error)) {
        ...
        this._log([
          `Error fetching data [${request.url}]: ${axiosError.message}`,
        ]);
