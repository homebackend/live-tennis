...
export abstract class MenuRendererCommon<
  T,
  TT,
  IT,
  PI,
  ...
  CI,
  MI,
  E extends PopupSubMenuItem<PI, LI | CI | MI>,
  L extends MenuItem<LI>,
  C extends CheckedMenuItem<CI>,
  M extends MatchMenuItem<MI>,
> extends Runner {
  private _EConstructor: new (
    properties: PopubSubMenuItemProperties,
    renderer: Renderer<T, TT, IT>
  ) => E;
  private _LConstructor: new (
    properties: LinkMenuItemProperties,
    renderer: Renderer<T, TT, IT>
  ) => L;
  private _CConstructor: new (
    properties: CheckedMenuItemProperties,
    renderer: Renderer<T, TT, IT>
  ) => C;
  private _MConstructor: new (
  ...
  handleMenuHidden(): void {
    Array.from(this._tournamentHeaders.values()).forEach((element) =>
      element.hide()
    );
  }
    ...
    const submenuItem = new this._EConstructor(
      {
        ...
        url: url,
        clickHandler: () =>
          this._tournamentHeaders.forEach((submenuItem) => submenuItem.hide()),
      },
      this._renderer
    );
