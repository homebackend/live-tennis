...
export class LiveViewUpdater<TF extends TTFetcher> {
  ...
  constructor(
    ...
    this._log = log;
    const atpFetcher = new AtpFetcher(
      'atp' in apiHandlers ? apiHandlers.atp : apiHandlers
    );
    const wtaFetcher = new WtaFetcher(
      'wta' in apiHandlers ? apiHandlers.wta : apiHandlers
    );
    const ttFetcher = new TFConstructor(
      'tt' in apiHandlers ? apiHandlers.tt : apiHandlers,
      log
    );
    this._liveTennis = new LiveTennis(
      log,
      settings,
      atpFetcher,
      wtaFetcher,
  ...
  private async _fetchMatchData() {
    try {
      ...
      const interval =
        await this._liveTennis.updateNextRunTimesAndGetInterval();
  ...
  private async _getSelectedMatches(matchesData: TennisMatch[]) {
    ...
    const onlyLiveMatches = await this._settings.getBoolean(
    ...
    const filteredMatchData = matchesData.filter((m) => {
       ...
       */
      return (
        this._runner.isMatchWaitingDeselection(matchId) ||
    ...
    this._log([
      ...
      selectedMatchIds.length.toString(),
      filteredMatchData.length.toString(),
    ]);
    return filteredMatchData;
  }

  private async _shouldHideLiveView(
    selectedMatches: TennisMatch[]
  ): Promise<boolean> {
    return (
  ...
  private async _updateFloatingWindows(matchesData: TennisMatch[]) {
    ...
    if (selectedMatches.length > numWindows) {
      ...
      selectedMatches.forEach((match, i) =>
        this._manager.updateLiveViewContent(i, match)
      );
  ...
  private async _cycleMatches(matchesData: TennisMatch[]) {
    const cycle = async () => {
      ...
      for (let i = 0; i < this._manager.getLiveViewCount(); i++) {
        const matchToShow =
          selectedMatches[
            (this._currentMatchIndex + i) % selectedMatches.length
          ];
        this._manager.updateLiveViewContent(i, matchToShow);
      }
      this._currentMatchIndex =
        (this._currentMatchIndex + 1) % selectedMatches.length;
      return this._manager.continueCycleTimeout();
    };

    await cycle();
    this._manager.destroyCycleTimeout();
    this._manager.setCycleTimeout(
      await this._settings!.getInt('match-display-duration'),
      cycle
    );
  }
}
